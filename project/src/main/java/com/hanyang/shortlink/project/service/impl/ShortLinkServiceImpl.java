...
import com.hanyang.shortlink.project.config.SkipDomainWhiteListConfiguration;
import com.hanyang.shortlink.project.dao.entity.*;
import com.hanyang.shortlink.project.dao.mapper.*;
import com.hanyang.shortlink.project.dto.biz.ShortLinkStatsRecordDTO;
...
import com.hanyang.shortlink.project.dto.resp.*;
import com.hanyang.shortlink.project.mq.producer.ShortLinkStatsSaveProducer;
...
public class ShortLinkServiceImpl extends ServiceImpl<ShortLinkMapper, ShortLinkDO> implements ShortLinkService {
    ...
    private final SkipDomainWhiteListConfiguration skipDomainWhiteListConfiguration;
    ...
    public void updateShortLink(ShortLinkUpdateReqDTO requestParam) {
        ...
        if (Objects.equals(hasShortLinkDO.getGid(), requestParam.getGid())) {
            ...
            try {
                ...
                LambdaQueryWrapper<ShortLinkSkipDO> linkSkipQueryWrapper = Wrappers.lambdaQuery(ShortLinkSkipDO.class)
                        .eq(ShortLinkSkipDO::getFullShortUrl, requestParam.getFullShortUrl())
                        .eq(ShortLinkSkipDO::getGid, hasShortLinkDO.getGid());
                ShortLinkSkipDO shortLinkSkipDO = shortLinkSkipMapper.selectOne(linkSkipQueryWrapper);
                shortLinkSkipMapper.deleteById(shortLinkSkipDO.getId());
                shortLinkSkipDO.setGid(requestParam.getGid());
                shortLinkSkipMapper.insert(shortLinkSkipDO);
        ...
        if (!Objects.equals(hasShortLinkDO.getValidDateType(), requestParam.getValidDateType())
            ...
            stringRedisTemplate.delete(String.format(SKIP_SHORT_LINK_KEY, requestParam.getFullShortUrl()));
            if (hasShortLinkDO.getValidDate() != null && hasShortLinkDO.getValidDate().before(new Date())) {
                if (Objects.equals(requestParam.getValidDateType(), ValidDateTypeEnum.PERMANENT.getType()) || requestParam.getValidDate().after(new Date())) {
                    stringRedisTemplate.delete(String.format(SKIP_IS_NULL_SHORT_LINK_KEY, requestParam.getFullShortUrl()));
                }
            }
        }
    }
    ...
    @SneakyThrows
    @Override
    public void recoverUrl(String shortUri, ServletRequest request, ServletResponse response) {
    ...
    private String generateSuffix(ShortLinkCreateReqDTO requestParam) {
        ...
        while (true) {
            ...
            if (!shortUrlCreateCachePenetrationBloomFilter.contains(createShortLinkDefaultDomain + "/" + shortUri)) {
                break;
            }
            customGenerateCount++;
        }
    ...
    private void verificationWhitelist(String originUrl) {
        Boolean enable = skipDomainWhiteListConfiguration.getEnable();
        ...
        List<String> details = skipDomainWhiteListConfiguration.getDetails();
        if (!details.contains(domain)) {
            throw new ClientException("演示环境为避免恶意攻击，请生成以下网站跳转链接：" + skipDomainWhiteListConfiguration.getNames());
        }
    }
}
